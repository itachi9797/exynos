...
var ProcessManager = class {
  constructor(message2, content, exynos, options = {}) {
  ...
  filterSecret(string) {
    string = string.replace(
      new RegExp(this.exynos.client.token, "gi"),
      "[accesstoken was hidden]"
    );
    if (this.exynos.options.secrets) {
      for (const el of this.exynos.options.secrets) {
...
async function main(message2, parent2) {
  ...
  let summary = `Exynos v${version}, discord.js \`${import_discord4.version}\`, \`Node.js ${process.version}\` on \`${process.platform}\`
...
`, parent2, {
  ...
  await msg2.addAction(
    [
      {
        button: new import_discord5.ButtonBuilder().setStyle(import_discord5.ButtonStyle.Danger).setCustomId("exynos$prev").setLabel("Prev"),
        action: ({ manager }) => manager.previousPage(),
        requirePage: true
      },
      {
        button: new import_discord5.ButtonBuilder().setStyle(import_discord5.ButtonStyle.Secondary).setCustomId("exynos$stop").setLabel("Stop"),
      ...
      {
        button: new import_discord5.ButtonBuilder().setStyle(import_discord5.ButtonStyle.Success).setCustomId("exynos$next").setLabel("Next"),
...
async function js(message, parent) {
  ...
  await msg.addAction([
    {
      button: new import_discord6.ButtonBuilder().setStyle(import_discord6.ButtonStyle.Danger).setCustomId("exynos$prev").setLabel("Prev"),
      action: ({ manager }) => manager.previousPage(),
      requirePage: true
    },
    {
      button: new import_discord6.ButtonBuilder().setStyle(import_discord6.ButtonStyle.Secondary).setCustomId("exynos$stop").setLabel("Stop"),
      action: ({ manager }) => manager.destroy(),
      requirePage: true
    },
    {
      button: new import_discord6.ButtonBuilder().setStyle(import_discord6.ButtonStyle.Success).setCustomId("exynos$next").setLabel("Next"),
      action: ({ manager }) => manager.nextPage(),
      requirePage: true
    }
  ]);
...
${inspect(value, {
  ...
  await msg2.addAction([
    {
      button: new import_discord7.ButtonBuilder().setStyle(import_discord7.ButtonStyle.Primary).setCustomId("exynos$prev").setLabel("Prev"),
      action: ({ manager }) => manager.previousPage(),
      requirePage: true
    },
    {
      button: new import_discord7.ButtonBuilder().setStyle(import_discord7.ButtonStyle.Secondary).setCustomId("exynos$stop").setLabel("Stop"),
      action: ({ manager }) => manager.destroy(),
      requirePage: true
    },
    {
      button: new import_discord7.ButtonBuilder().setStyle(import_discord7.ButtonStyle.Success).setCustomId("exynos$next").setLabel("Next"),
      action: ({ manager }) => manager.nextPage(),
      requirePage: true
    }
  ]);
...
${table({
  ...
  await msg.addAction([
    {
      button: new import_discord8.ButtonBuilder().setStyle(import_discord8.ButtonStyle.Danger).setCustomId("exynos$prev").setLabel("Prev"),
      action: ({ manager }) => manager.previousPage(),
      requirePage: true
    },
    {
      button: new import_discord8.ButtonBuilder().setStyle(import_discord8.ButtonStyle.Secondary).setCustomId("exynos$stop").setLabel("Stop"),
      action: ({ manager }) => manager.destroy(),
      requirePage: true
    },
    {
      button: new import_discord8.ButtonBuilder().setStyle(import_discord8.ButtonStyle.Success).setCustomId("exynos$next").setLabel("Next"),
      action: ({ manager }) => manager.nextPage(),
      requirePage: true
    }
  ]);
...
async function curl(message2, parent2) {
  ...
  await msg2.addAction([
    {
      button: new import_discord9.ButtonBuilder().setStyle(import_discord9.ButtonStyle.Danger).setCustomId("exynos$prev").setLabel("Prev"),
      action: ({ manager }) => manager.previousPage(),
      requirePage: true
    },
    {
      button: new import_discord9.ButtonBuilder().setStyle(import_discord9.ButtonStyle.Secondary).setCustomId("exynos$stop").setLabel("Stop"),
      action: ({ manager }) => manager.destroy(),
      requirePage: true
    },
    {
      button: new import_discord9.ButtonBuilder().setStyle(import_discord9.ButtonStyle.Success).setCustomId("exynos$next").setLabel("Next"),
      action: ({ manager }) => manager.nextPage(),
      requirePage: true
    }
  ]);
...
async function cat(message2, parent2) {
  ...
  import_fs.default.readFile(filename, async (err, data) => {
    ...
    await msg2.addAction([
      {
        button: new import_discord10.ButtonBuilder().setStyle(import_discord10.ButtonStyle.Danger).setCustomId("exynos$prev").setLabel("Prev"),
        action: ({ manager }) => manager.previousPage(),
        requirePage: true
      },
      {
        button: new import_discord10.ButtonBuilder().setStyle(import_discord10.ButtonStyle.Secondary).setCustomId("exynos$stop").setLabel("Stop"),
        action: ({ manager }) => manager.destroy(),
        requirePage: true
      },
      {
        button: new import_discord10.ButtonBuilder().setStyle(import_discord10.ButtonStyle.Primary).setCustomId("exynos$next").setLabel("Next"),
        action: ({ manager }) => manager.nextPage(),
        requirePage: true
      }
    ]);
...
var Exynos = class {
  /**
   * Main Client of Exynos
   * @param client Discord Client
   * @param options Exynos Options
   */
  constructor(client2, options) {
    ...
    if (!this.options.aliases)
      this.options.aliases = ["exynos", "dok"];
    this.process = [];
    client2.once("ready", (client3) => {
      if (!this.owners.length) {
        console.warn("[ Exynos ] Owners not given. Fetching from Application.");
        client3.application.fetch().then((data) => {
          if (!data.owner) {
            return console.warn("[ Exynos ] Falied to owner data.");
          }
          ...
          console.info(
            `[ Exynos ] Fetched ${this.owners.length} owner(s): ${this.owners.length > 3 ? this.owners.slice(0, 3).join(", ") + ` and ${this.owners.length - 3} more owners` : this.owners.join(", ")}`
          );
        });
      }
    });
